...
import ImageViewerWidget from './MayoImageViewerBase.js';
...
const extended = {
            ...
            feature.geoOn(geo.event.pan, () => {
                if (this.viewer.zoom() !== annotation._centroidLastZoom) {
                    ...
                    if (feature.verticesPerFeature) {
                        ...
                        const vpf = feature.verticesPerFeature(), count = feature.data().length, radius = new Float32Array(vpf * count);
                ...
                if (annotation._centroids && featureList[0]) {
                    if (featureList[0].verticesPerFeature) {
                        this.viewer.scheduleAnimationFrame(() => {
                            const vpf = featureList[0].verticesPerFeature(), count = featureList[0].data().length, shown = new Float32Array(vpf * count);
    ...
    setBounds: function (annotations) {
        var zoom = this.viewer.zoom(), bounds = this.viewer.bounds(), zoomRange = this.viewer.zoomRange();
...
var MayoImageViewer = ImageViewerWidget.extend(extended);

export default MayoImageViewer;
