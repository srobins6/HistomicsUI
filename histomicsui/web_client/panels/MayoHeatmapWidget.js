...
import mayoHeatmapWidget from '../templates/panels/mayoHeatmapWidget.pug';
import '../stylesheets/panels/mayoHeatmapWidget.styl';

var HeatmapWidget = Panel.extend({
    events: _.extend(Panel.prototype.events, {
        'change .h-upload-button': '_uploadHeatmap',
    }),
    render() {
        this.getHeatmapIdPromise = restRequest({
            url: `mayoHeatmap/getHeatmap/${this.image.id}`,
            method: 'GET',
            error: null,
        }).done((result) => {
            this.heatmapId = result;
            // var $heatmappreview = $("#heatmap-preview");
            // const heatmapSource = 'item/' + this.heatmapId + '/tiles/thumbnail';
            // $heatmappreview.attr("src", heatmapSource)
            this._setHeatmap(result);
        });
    },
    ...
    _setHeatmap(heatmapId) {
        this.$el.html(mayoHeatmapWidget({
    ...
    _uploadHeatmap() {
        ...
        const $button = $('#heatmap-upload-button');
        var file = $button.prop('files')[0];
        file = new File([file], `${this.image.id}_heatmap.svs`);
        const fileModel = new FileModel();
        fileModel.upload(this.image.parent, file);


        // var heatmapData = $form.serializeArray();
        // console.log($form);
        // console.log(heatmapData);
        // this.saveHeatmapPromise = restRequest({
        //     url: `mayoHeatmap/saveHeatmap/${this.image.id}`,
        //     method: 'POST',
        //     error: null,
        //     data: {
        ...
        // var $uploadButton = $('.h-upload-button');
        // if ($uploadButton.prop('files').length > 0) {
        //     var file = $uploadButton.prop('files')[0];
        //     formdata.append("heatmap", file);
        //     $.ajax({
        //         url: `/api/v1/mayoHeatmap/saveHeatmap/${this.image.id}`,
        //         type: "POST",
        //         data: formdata,
        //         processData: false,
        //         contentType: false,
